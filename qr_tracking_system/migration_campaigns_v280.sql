...
SELECT
    ...
    COUNT(CASE WHEN s.operating_system ILIKE '%ios%' THEN 1 END) * 100.0 /
        NULLIF(COUNT(s.id), 0)                           AS ios_pct,
    ROUND(AVG(s.device_pixel_ratio)::numeric, 2)                  AS avg_dpr,
    ROUND(AVG(s.cpu_cores)::numeric, 1)                           AS avg_cpu_cores,
    -- Temporal
    c.start_date,
    c.end_date,
