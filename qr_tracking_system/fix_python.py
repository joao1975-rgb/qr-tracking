    ...
    with get_db_connection() as conn:
        ...
        cursor.execute(\"\"\"
            SELECT 
                ...
                COUNT(CASE WHEN browser ILIKE '%%ios%%' OR operating_system ILIKE '%%ios%%' THEN 1 END) * 100.0 / NULLIF(COUNT(id), 0) as ios_pct
...
async def compare_vs_benchmark(campaign_code: str):
    with get_db_connection() as conn:
        ...
        if not best_in_group:
            return {"status": "no_benchmark", "message": "No hay datos de benchmark suficientes para esta categoría"}
        ...
        # Simplificando lógica de percentil
...
async def compare_vs_selected(campaign_code: str, compare_code: str):
    with get_db_connection() as conn:
        ...
        # Si no es el mismo cliente, anonimizamos datos de 'compare'
        comp_data = {
...
start_cut = app_code.find('@app.post("/api/campaigns")')
end_cut = app_code.find('@app.put("/api/campaigns/{campaign_code}/pause")')
if start_cut != -1 and end_cut != -1:
    app_code = app_code[:start_cut] + create_update_code + app_code[end_cut:]
    
start_endp = app_code.find('# SECCIÓN 5: ENDPOINTS NUEVOS')
if start_endp != -1:
    app_code = app_code[:start_endp] + analytics_part
else:
    app_code += '\\n\\n' + analytics_part
